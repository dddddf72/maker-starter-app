...
const ConfirmPinView = ({
  ...
  return (
    ...
    >
      <Box flex={1} />
      <Text marginBottom="m" variant="h1" maxFontSizeMultiplier={1} color="white">
        {title}
      </Text>

      {/* <Text variant="body1" color="white">{subtitle}</Text> */}
      <Animated.View style={{ transform: [{ translateX: shakeAnim.current }] }}>
        <PinDisplay length={pin.length} marginVertical="xl" />
      </Animated.View>
      <Keypad
        customButtonTitle={clearable ? t('generic.clear_1') : t('generic.cancel_1')}
